        ...
        .then((response) => {
            ...
            if( response.dataValues.balance >= value){
                ...
                .then((responseB) => {
                    ...
                    return Account.update({balance: cantidadRecive}, {where: {cvu: destination}})
                })
                .then((responseC) => {
                    //crear modelo trasfer para registrar estas transferencias.
                    // console.log("AAAAAAAAAAAAA 4")
                    Transfer.create({origin, destination, value, type, currency, description })
                })
                .then((responseD) => {
...
server.post("/deposito", (req, res) => {
    ...
    .then((responseA) => {
        Transfer.create({origin: 0, destination: cvu, value, type: "DEP", currency, description : "Deposito de usuario."})
...
server.post("/retiro", (req, res) => {
    ...
    .then((responseA) => {
        Transfer.create({origin: cvu,destination: 0, value, type: "EXT", currency, description : "Retiro de usuario."})
